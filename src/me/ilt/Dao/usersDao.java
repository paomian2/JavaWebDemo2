...
public class usersDao {
	...
	//	String sql = "select  "+pageSize+" * from t_user where id not in(select top "+(p-1)*pageSize+" id from t_user)";
		String sql="select * from t_user limit "+p+","+pageSize;
		return sel(sql);
	}
	/**
	 * 根据昵称查询
	...
	public static JSONArray nameSel(int p,int pageSize,String username){
	   ...
	   String sql="select * from t_user where userName like '%"+username+"%' limit "+p+","+pageSize ;
		return sel(sql);
	}
	/**
	 * 查询sql 返回json集合
