...
public class goodsDao {
	...
	public static List salesTop(){
		String sql = "select id,name,price,proPic from t_goods  order by sales desc limit 0,5";
	...
	public static PageBean sidPageSel(String sid,int p,String order){
		...
		String sql2 = null;
		sql2 = "select id,name,price,proPic "
				+"from t_goods "
				+"where smallTypeId ="+sid
				+" order by views desc limit 0,"+pb.getPagesize() ;
//		if(order.equals("1")){
//			sql2 = "select id,name,price,proPic "
//				+"from t_goods "
//				+"where smallTypeId ="+sid
//				+" and name not in(select name from t_goods where smallTypeId ="+sid+" order by views desc limit 0,"+(pb.getP()-1)*pb.getPagesize()+") "
//				+"order by views desc limit 0,"+pb.getPagesize() ;
//		}else if(order.equals("2")){
//			sql2 = "select id,name,price,proPic "
//					+"from t_goods "
//					+"where smallTypeId ="+sid
//					+" and name not in(select name from t_goods where smallTypeId ="+sid+" order by id desc limit 0, "+(pb.getP()-1)*pb.getPagesize()+") "
//					+"order by id desc limit 0,"+pb.getPagesize();
//		}else if(order.equals("3")){
//			sql2 = "select id,name,price,proPic "
//					+"from t_goods "
//					+"where smallTypeId ="+sid
//					+" and name not in(select top "+(pb.getP()-1)*pb.getPagesize()+" name from t_goods where smallTypeId ="+sid+" order by price desc) "
//					+"order by price desc limit 0, "+pb.getPagesize()+" ";
//		}else if(order.equals("4")){
//			sql2 = "select id,name,price,proPic "
//					+"from t_goods "
//					+"where smallTypeId ="+sid
//					+" and name not in(select name from t_goods where smallTypeId ="+sid+" order by sales desc limit 0,"+(pb.getP()-1)*pb.getPagesize()+") "
//					+"order by sales desc limit 0, "+pb.getPagesize()+"";
//		}
	...
	public static List bigTypeIdSelxg(int bigTypeId){
		String sql = "select id,name,price,proPic from t_goods where bigTypeId="+bigTypeId+"  order by sales desc limit 0,6";
		Connection con = Conn.getCon();
		ResultSet rs = null;
		List<goodsBean> list = new ArrayList<goodsBean>();
		try {
...
public static goodsBean gwcGoodsIdSel(int id){
	...
	public static List bigTypeIdSel(int bigTypeId){
		String sql = "select id,name,price,proPic from t_goods where bigTypeId="+bigTypeId+" order by id desc limit 0,10 ";
		Connection con = Conn.getCon();
		ResultSet rs = null;
		List<goodsBean> list = new ArrayList<goodsBean>();
		try {
...
//		String sql = "select  "+pageSize+" t_goods.*,t_smalltype.name smallTypeName,t_bigtype.name bigTypeName "
...
//				+"and t_goods.id not in( select top "+(p-1)*pageSize+" t_goods.id from t_goods where name like '%"+name+"%' order by t_goods.id desc ) order by t_goods.id desc";
		...
		String sql = "select t_smalltype.name smallTypeName,t_bigtype.name bigTypeName "
				+"from t_goods,t_smalltype,t_bigtype where t_smalltype.id = t_goods.smallTypeId and t_goods.name like '%"+name+"%' and t_bigtype.id = t_goods.bigTypeId "
				+" order by t_goods.id desc limit 0,"+pageSize;
		return sel(sql);
	}
...
//				+") order by t_goods.id desc";
		...
		String sql = "select t_smalltype.name smallTypeName,t_bigtype.name bigTypeName from t_goods,t_smalltype,t_bigtype "
				+"where t_smalltype.id = t_goods.smallTypeId and t_bigtype.id = t_goods.bigTypeId order by t_goods.id desc limit 0,"+pageSize;
		return sel(sql);
	}
	/**
	 * 查询总行数
